...
export default {
    name: 'UnderwriterApprovalDetail',
    emits: ['reloadTableData'],
    props: {
        btnName: String,
        divisionDropdownLs: Object,
    },
    setup() {
        ...
        const data = computed(() => store.state.loadingrange.loadingRangeDetail);

        const inputForm = ref({
            seqId: data.value.seqId,
            loadCode: data.value.loadCode,
            effectiveDateFrom: new Date(data.value.effectiveDateFrom),
            effectiveDateTo: new Date(data.value.effectiveDateTo),
            minLoadRate: data.value.minLoadRate,
            maxLoadRate: data.value.maxLoadRate,
        });
        ...
        const saveExit = () => {    
            ...
            if(checkError().length > 0){
                ...
                store.dispatch('loadingrange/updateLoadingRange', inputForm.value).then(() => {
        ...
        const delRecord = () => {    
            ...
            if(answer){
                store.dispatch('loadingrange/deleteLoadingRange', inputForm.value.seqId);
