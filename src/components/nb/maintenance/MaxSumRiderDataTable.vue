<template>
    ...
    >
        ...
        <Column field="minSumAssured" header="Min Sum Assured"></Column>
        <Column field="maxSumAssured" header="Max Sum Assured"></Column>
...
import MaxSumRiderAddModal from '@/components/nb/maintenance/MaxSumRiderAddModal';

export default {
    name: 'MaxSumRiderDataTable',
    components: {
        MaxSumRiderAddModal
    },
    props: {
    ...
    setup(props, { emit }) {
        ...
        const maxSumRiderLs = ref({});
        const searchForm = toRef(props, 'inputForm');

        onBeforeUpdate(() => {

            masterDataService.getDetailByHeaderCode('JETMAXSATBL').then(res => {
                maxSumRiderLs.value = res.data;
            });
        })
