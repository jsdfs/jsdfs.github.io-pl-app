...
export default {
    ...
    setup() {
        ...
        const data = computed(() => store.state.exclusion.exclusionCodeDetail);
        const store = useStore();
        const addExclusionObj = ref({});
        const inputForm = ref({
            seqId: data.value.seqId,
            division: data.value.division,
            exclusionCode: data.value.exclusionCode,
            effectiveDateFrom: new Date(data.value.effectiveDateFrom),
            effectiveDateTo: new Date(data.value.effectiveDateTo),
            descriptionEnglish: data.value.descriptionEnglish,
            descriptionBm: data.value.descriptionBm
        });
        ...
        const saveExit = () => {    
            ...
            if(checkError().length > 0){
                ...
                store.dispatch('exclusion/updateExclusionCode', inputForm.value).then(() => {
        ...
        const delRecord = () => {    
            ...
            if(answer){
                store.dispatch('exclusion/deleteExclusionCode', inputForm.value.seqId);
